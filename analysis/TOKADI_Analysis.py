...
"""
TOKADI - Auswertung Paper A (Artefakt-Evaluation)

VERWENDUNG
  TOKADI_Analyse.py und TOKADI_Datensatz_final.xlsx in denselben Ordner legen:
      python TOKADI_Analyse.py
  Ausgabe in Datei:
      python TOKADI_Analyse.py > auswertung.txt
  Anderer Ordner:
      python TOKADI_Analyse.py "C:\Pfad\zum\ordner"

BENÖTIGT:  pandas, numpy, scipy, openpyxl
      pip install pandas numpy scipy openpyxl

Förderbedarf = Definition B (Förderschwerpunkte inkl. Sternchen-Fälle, ohne DaF/DAZ)
"""
...
# Funktion zum Lesen von Sheets mit/ohne Umlauten
def read_sheet_umlaut(excel_file, sheet_names):
    """Versucht Sheets mit und ohne Umlaute zu lesen."""
    for name in sheet_names:
        try:
            return pd.read_excel(excel_file, sheet_name=name)
        except ValueError:
            continue
    # Wenn nichts gefunden wurde, zeige verfügbare Sheets
    available = pd.ExcelFile(excel_file).sheet_names
    raise ValueError(f"Keines der Sheets {sheet_names} gefunden. Verfügbare Sheets: {available}")

# Funktion zum Finden von Spalten mit/ohne Umlauten
def get_column(df, col_names):
    """Findet eine Spalte mit verschiedenen Schreibweisen."""
    for name in col_names:
        if name in df.columns:
            return name
    raise KeyError(f"Keine der Spalten {col_names} in {df.columns.tolist()}")

# Sheets mit beiden Schreibweisen versuchen
LK  = read_sheet_umlaut(F, ['01_Lehrkräfte', '01_Lehrkraefte'])
LKD = read_sheet_umlaut(F, ['01b_LK_Demografie', '01b_LK_Demographie'])
LKD = LKD[LKD['LK_ID'].astype(str).str.fullmatch(r'T\d+')].copy()
SEN_COL = next(c for c in LKD.columns if c.startswith('SEN_in_Klasse'))
for _c in ['Alter', 'Schüler_in_Klasse', 'Schueler_in_Klasse', SEN_COL, 'Experience (yrs)']:
    if _c in LKD.columns: LKD[_c] = pd.to_numeric(LKD[_c], errors='coerce')
SM  = read_sheet_umlaut(F, ['02_Schüler_Motivation', '02_Schueler_Motivation'])
GA  = read_sheet_umlaut(F, ['03_Schüler_Gruppenarbeit', '03_Schueler_Gruppenarbeit'])
print(f"Gelesen: {len(LK)} Lehrkräfte, {len(SM)} Schüler (Motivation), {len(GA)} Schüler (Gruppenarbeit)")
...
# Prüfen, ob Spalte 'Schüler_in_Klasse' oder 'Schueler_in_Klasse' existiert
schueler_col = 'Schüler_in_Klasse' if 'Schüler_in_Klasse' in LKD.columns else 'Schueler_in_Klasse'
print(f"Kinder in den Klassen gesamt: {LKD[schueler_col].sum():.0f} | von LK berichtete SEN-Kinder: {LKD[SEN_COL].sum():.0f}")
print("   (Summen über Lehrkräfte; je Klasse sind zwei Lehrkräfte befragt, Klassenwerte also doppelt gezählt)")
print(f"Erhebung: Pre {LKD.Pretest.min()} | Post {LKD.Posttest.max()}")
if 'Experience (yrs)' in LKD.columns and LKD['Experience (yrs)'].notna().any():
    _e = LKD['Experience (yrs)'].dropna()
    print(f"Berufserfahrung: M={_e.mean():.1f} SD={_e.std():.1f} range {_e.min():.0f}-{_e.max():.0f} (n={len(_e)})")
else:
    print("HINWEIS Berufserfahrung: Der Lehrkräftefragebogen erhebt die Berufserfahrung,")
...
# ── 3 UMUX ──
H("3  UMUX  (7-stufig, Format nach Finstad 2010)")
Uv=LK[[f'UMUX_{i}' for i in range(1,5)]].values
# Auswertung: negative Items 2 und 4 umpolen (8 - Antwort),
# positive Items unverändert, dann (Summe - 4) / 24 * 100
umux=np.array([((r[0]+(8-r[1])+r[2]+(8-r[3]))-4)/24*100 for r in Uv])
m,s,lo,hi=ci95(umux)
print(f"N={len(umux)} M={m:.2f} SD={s:.2f} Min={umux.min():.1f} Max={umux.max():.1f} Median={np.median(umux):.2f} 95%CI[{lo:.2f},{hi:.2f}]")
print("Einzelwerte:", [round(v,2) for v in sorted(umux.tolist())])
print("Rohwertebereich der vier Items:", int(Uv.min()), "bis", int(Uv.max()), "(Skala 1-7)")
Urec=np.column_stack([Uv[:,0],8-Uv[:,1],Uv[:,2],8-Uv[:,3]])
...
# Robustheit gegen schwache Subskalen-Reliabilität: Vergleich auf Ebene der einzelnen
...
# ── 5 Schüler ──
H("5  TABELLE 1 - SCHÜLERSTICHPROBE")
print(f"N = {len(SM)}")
print(f"Alter: M={SM.Alter.mean():.2f} SD={SM.Alter.std():.2f} range {SM.Alter.min():.0f}-{SM.Alter.max():.0f}")
g=SM.Geschlecht.value_counts(); print(f"Geschlecht: Mädchen {g.get('Girl',0)} ({g.get('Girl',0)/len(SM)*100:.1f}%), Jungen {g.get('Boy',0)} ({g.get('Boy',0)/len(SM)*100:.1f}%)")
...
print(f"\nSonderpäd. Förderbedarf (Def. B): {int(SM.SEN_DefB.sum())} ({SM.SEN_DefB.mean()*100:.1f}%)")
# Spalte 'Förderung_Original' mit/ohne Umlaut finden
foerderung_col = get_column(SM, ['Förderung_Original', 'Foerderung_Original'])
print("   ", dict(SM.loc[SM.SEN_DefB==1, foerderung_col].value_counts()))
# Spalte 'Sprachförderung' mit/ohne Umlaut finden
sprachfoerderung_col = get_column(SM, ['Sprachförderung', 'Sprachfoerderung'])
print(f"Sprachförderung (DaF/DAZ, separat): {int(SM[sprachfoerderung_col].sum())} ({SM[sprachfoerderung_col].mean()*100:.1f}%)")

H("6  SCHÜLERFRAGEBOGEN")
...
cmp(SM.SEN_DefB,"mit ","ohne","Förderbedarf (Def. B)")
...
lab={1:"Ich mochte die Aktivität",2:"Ich hatte Spaß an der Aktivität",3:"Wir haben uns gegenseitig geholfen",
     4:"Ich habe gut zugehört und mich gehört gefühlt",5:"Ich fühlte mich wohl in meinem Team",
     6:"Ich hatte ein gutes Gefühl",7:"Ich mochte die Programmierung des Roboters",
     8:"Ich habe gerne mit Teamkollegen zusammengearbeitet",9:"Neue Art von Aktivität spannend"}
...
print("\nEFA-Prüfung der a-priori-Dimensionen:")
print("   Eigenwerte:", np.round(ev,2))
print(f"   Faktoren EW>1: {int((ev>1).sum())} | F1 erklärt {ev[0]/9*100:.1f}%")
for nm,ii in [("D1 Individuelle positive Affektivität",[1,2,6]),("D2 Soziale Eingebundenheit & Teamkohäsion",[3,5,8]),
...
# ── 10 SENSITIVITÄTSANALYSE: Operationalisierungen A-D ──
H("10  SENSITIVITÄTSANALYSE - Operationalisierung von Benachteiligung (A-D)")
print("Der zentrale Gruppenunterschied (Förderbedarf vs. übrige) wird gegen vier")
print("zunehmend breitere Operationalisierungen geprüft. 'ges' = mittlere Zufriedenheit")
print("(Items 1-5, Schüler-Motivation). Welch-t (ungleiche Varianzen), Cohens d,")
print("Bootstrap-95%-KI der Mittelwertdifferenz (10 000 Resamples).\n")
foerderung_col = get_column(SM, ['Förderung_Original', 'Foerderung_Original'])
orig = SM[foerderung_col].fillna('').astype(str)
star = orig.str.contains(r'\*')
defA = ((SM.SEN_DefB==1) & (~star)).astype(int)                       # nur eindeutige Förderschwerpunkte
defB = SM.SEN_DefB.astype(int)                                        # Hauptanalyse (inkl. Sternchen-Fälle)
# Spalte 'Sprachförderung' mit/ohne Umlaut für defC und defD
sprachfoerderung_col = get_column(SM, ['Sprachförderung', 'Sprachfoerderung'])
defC = ((SM.SEN_DefB==1) | (SM[sprachfoerderung_col]==1)).astype(int)      # + Sprachförderung (DaF/DAZ)
defD = ((SM.SEN_DefB==1) | (SM[sprachfoerderung_col]==1) |
...
print(f"{'Operationalisierung':40s}{'n(benacht.)':>12s}{'M_benacht':>10s}{'M_übrig':>9s}{'Welch-t':>9s}{'p':>8s}{'d':>7s}   Boot-95%-KI")
for lab, mask in [("A  nur eindeutige Förderschwerpunkte", defA),
                  ("B  + Sternchen-Fälle (Hauptanalyse)", defB),
                  ("C  + Sprachförderung (DaF/DAZ)",       defC),
...
print("\nLesart: Bleibt das Vorzeichen und die Größenordnung von d über A-D stabil,")
print("ist der Befund robust gegen die Wahl der Förderbedarfs-Definition.")

# ── 11 ROBUSTHEITSBATTERIE für Tabelle 9 ──
...
comps = [("Geschlecht (w vs m)",        (SM.Geschlecht=='Girl').astype(int)),
         ...
         ("Förderbedarf (Def. B)",     SM.SEN_DefB.astype(int)),
...
# Leave-one-out für den signifikanten SEN-Befund
H("11b  LEAVE-ONE-OUT - Stabilität des Förderbedarf-Befunds (Def. B)")
...
print(f"   p-Bereich: {min(ps_loo):.3f} - {max(ps_loo):.3f} | weiterhin p<.05 in {sum(p<.05 for p in ps_loo)}/{len(ps_loo)} Fällen")
...
print(f"ICC(1) = {icc:.3f} | mittlere Clustergröße n0 = {n0:.2f} | Designeffekt = {deff:.2f}")
